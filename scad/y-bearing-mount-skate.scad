...
nu_Y_bearing_holder_height=25.4;
...
// Move to center points of each skate bearing center
module skate_bearing_centers() {
	for (side=[-1,+1])
			scale([side,side,1]) translate([0,-13,0])
				rotate([0,-50,0]) children();
}

module roller_bearing_holder(diameter, bearing, height) {
		skate_OD=22.0;
		skate_ID=7.9;
		skate_thick=7.2;
		skate_clear=0.8;
	wall=4;
	W = bearing_holder_width(bearing);
    L = bearing_holder_length(bearing);
	H = height;
	intersection() {
		translate([0,0,100-height]) cube([200,200,200],center=true);
		difference() {
			union() {
			// Main body:
			translate([0,0,+H/2-height])
				cube(size = [W, L, H], center = true);
				
				// Reinforcing around bearings
				skate_bearing_centers() 
					translate([-diameter/2-skate_OD/2,0,0])
						cube([skate_OD+2*wall,skate_OD+3*wall,skate_thick+3*wall],center=true);
			}
			
			// Clearance for main shaft
			hull() for (off=[[-20,0,+20],[+20,0,+20],[0,0,0]])
				translate(off) rotate([90,0,0])
					cylinder(d=diameter+2,h=2*L+2,center=true);
			
			// Clearance for the skate bearings and skate shafts
			skate_bearing_centers()
				translate([-diameter/2-skate_OD/2,0,0])
					union() {
						
						// Skate bearings, with insertion path:
						hull() 
						for (insert=[-30,0]) translate([insert,0,0])
						cylinder(d=skate_OD+skate_clear*2,h=skate_thick+skate_clear,center=true);
						
						// Mounting shafts:
						translate([0,0,skate_thick/2+wall]) rotate([180,0,0]) cylinder(d=skate_ID,h=50);
					}
		}
	}
}
...
module bearing_mount(diameter, bearing, height) {
    ...
    color(y_bearing_mount_color) union() {
		roller_bearing_holder(diameter, bearing, height);
...
module y_bearing_mount_stl(diameter) translate([0,0, Y_bearing_holder_height]) bearing_mount(diameter, Y_bearings, nu_Y_bearing_holder_height);

module y_bearing_mounts_stl()
{
	ironpipe=21.5; // OD of iron pipe
	Y_bar_real=15.9; // OD of steel rod
	
    translate([0,  bearing_mount_length(Y_bearings)*1.5, 0]) 
    y_bearing_mount_stl(Y_bar_real);
    translate([0,  0, 0]) 
		y_bearing_mount_stl(ironpipe);
    translate([-(bearing_mount_width(Y_bearings) - 0.5*tab_length + 5), 0, 0]) 	
		y_bearing_mount_stl(ironpipe);
}
